...
function buildRecord(config, row, items, user) {
  ...
  const selfApprovalBlocked = !isAdmin(user) && requesterId && requesterId === Number(user.id);
  ...
  return {
    ...
    documentNumber: row.document_number || row.requisition_number || row.adjustment_number || row.voucher_number,
    requesterId,
    requesterName: row.requester_name || "Unknown requester",
    requestedDate: row.request_date || row.adjustment_date || row.payment_date || row.start_date || row.created_at,
    ...
    blockedReason: selfApprovalBlocked ? "You raised this request. Another approver must review it." : null,
  };
}
...
function createApprovalsRoutes(db) {
  ...
  router.get("/pending", requirePermission(...APPROVAL_PERMISSIONS), async (req, res, next) => {
    try {
      const [
        legacyRequisitions,
        goodsRequisitions,
      ...
      const itemRows = await Promise.all([
        Promise.all(
          legacyRequisitions.map((row) => db.all(
            `SELECT pri.id, p.name AS product_name, COALESCE(uom.code, uom.name) AS unit_code,
             ...
             ORDER BY p.name`, [row.id]
          ))
        ),
        Promise.all(
          goodsRequisitions.map((row) => db.all(
            `SELECT pri.id, p.name AS product_name, COALESCE(uom.code, uom.name) AS unit_code,
                    pri.quantity_requested, COALESCE(NULLIF(pri.quantity_approved, 0), pri.quantity_requested) AS approval_quantity,
                    pri.estimated_unit_cost, pri.preferred_supplier_id
             FROM purchase_requisition_items pri
        ...
        Promise.all(
          kitchenRequisitions.map((row) => db.all(
      ...
      const configs = {
        legacyRequisition: {
          ...
          approveBody: (items) => ({ items: items.map((item) => ({ id: item.id, quantityApproved: numeric(item.approval_quantity), estimatedUnitCost: numeric(item.estimated_unit_cost), preferredSupplierId: item.preferred_supplier_id || null })) }),
        },
        goodsRequisition: {
        ...
        kitchenRequisition: {
          ...
          approveBody: (items) => ({ items: items.map((item) => ({ id: item.id, approvedQuantity: numeric(item.approval_quantity) })) }),
        },
      };

      const records = [
        ...legacyRequisitions.map((row, index) => buildRecord(configs.legacyRequisition, row, itemRows[0][index], req.session.user)),
        ...goodsRequisitions.map((row, index) => buildRecord(configs.goodsRequisition, row, itemRows[1][index], req.session.user)),
        ...cashRequisitions.map((row) => buildRecord(configs.cashRequisition, row, [], req.session.user)),
        ...kitchenRequisitions.map((row, index) => buildRecord(configs.kitchenRequisition, row, itemRows[2][index], req.session.user)),
      ].filter(Boolean);
      ...
      res.json({
        ...
        meta: {
          ...
          actionable: records.filter((record) => record.canAct).length,
        },
      });
    } catch (error) {
      next(error);
